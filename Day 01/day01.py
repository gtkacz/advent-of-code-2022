...
def main() -> Union[str, int]:
    ...
    input_array: np.array = np.array([np.sum(np.array(line, dtype=object)) for line in list(map(lambda each: list(map(int, each.split('\n'))), input_data.split('\n\n')))], dtype=object)
    
    max_idx: int = np.argmax(input_array)

    return f'Elf number {max_idx} is carrying the most calories, with {input_array[max_idx]}'
